...
> **注意**：可自定义键盘快捷键需要 Claude Code v2.1.18 或更高版本。用 `claude --version` 检查你的版本。

Claude Code 支持自定义键盘快捷键。运行 `/keybindings` 创建或打开位于 `~/.claude/keybindings.json` 的配置文件。

---

## 配置文件

键绑定配置文件是一个带 `bindings` 数组的对象。每个块指定一个上下文和按键到操作的映射。

> **注意**：对键绑定文件的更改会自动检测并应用，无需重启 Claude Code。
...
这个示例将 `Ctrl+E` 绑定为在聊天上下文中打开外部编辑器，并解绑 `Ctrl+U`：
...
操作遵循 `namespace:action` 格式，如 `chat:submit` 发送消息或 `app:toggleTodos` 显示任务列表。每个上下文都有特定的可用操作。
...
\*在没有 VT 模式的 Windows 上（Node 低于 24.2.0 或 22.17.0，Bun 低于 1.2.23），默认为 Meta+M。
...
| 操作                    | 默认值 | 描述                                     |
| :---------------------- | :----- | :--------------------------------------- |
| `footer:next`           | Right  | 下一个底部项                             |
| `footer:previous`       | Left   | 上一个底部项                             |
| `footer:up`             | Up     | 在底部向上导航（顶部取消选择）           |
| `footer:down`           | Down   | 在底部向下导航                           |
| `footer:openSelected`   | Enter  | 打开选中的底部项                         |
| `footer:clearSelection` | Escape | 清除底部选择                             |
...
| `diff:previousFile`   | Up                 | diff 中的上一个文件  |
| `diff:nextFile`       | Down               | diff 中的下一个文件  |
| `diff:viewDetails`    | Enter              | 查看 diff 详情       |
| `diff:back`           | （上下文特定）     | 在 diff 查看器中返回 |
...
在启用[语音听写](/en/voice-dictation)时在 `Chat` 上下文中可用的操作：
...
在启用[全屏渲染](/en/fullscreen)时在 `Scroll` 上下文中可用的操作：
...
| `scroll:halfPageUp`     | （未绑定）           | 向上滚动视口高度的一半。与 `scroll:pageUp` 行为相同，用于 vi 风格重绑定                          |
| `scroll:halfPageDown`   | （未绑定）           | 向下滚动视口高度的一半。与 `scroll:pageDown` 行为相同，用于 vi 风格重绑定                        |
| `scroll:fullPageUp`     | （未绑定）           | 向上滚动整个视口高度                                                                             |
| `scroll:fullPageDown`   | （未绑定）           | 向下滚动整个视口高度                                                                             |
| `selection:copy`        | Ctrl+Shift+C / Cmd+C | 将选中文本复制到剪贴板                                                                           |
...
独立的大写字母隐含 Shift。例如，`K` 等效于 `shift+k`。这对于 vim 风格的绑定很有用，大写和小写键有不同的含义。

带修饰键的大写字母（如 `ctrl+K`）被视为风格化的，**不**隐含 Shift：`ctrl+K` 与 `ctrl+k` 相同。
...
```text
ctrl+k ctrl+s   按 Ctrl+K，松开，然后按 Ctrl+S
```
...
这也适用于组合键绑定。解绑共享前缀的每个组合键可以释放该前缀用作单键绑定：
...
如果你解绑了前缀上的一些但不是全部组合键，按下前缀仍然会进入剩余绑定的组合键等待模式。
...
当通过 `/config` → Editor mode 启用 vim 模式时，键绑定和 vim 模式独立运行：

- **Vim 模式**在文本输入级别处理输入（光标移动、模式、动作）
- **键绑定**在组件级别处理操作（切换待办、提交等）
- vim 模式中的 Escape 键将 INSERT 切换到 NORMAL 模式；它不会触发 `chat:cancel`
- 大多数 Ctrl+键 快捷键通过 vim 模式传递到键绑定系统
