...
工具允许 LLM 在您的代码库中执行操作。OpenCode 自带了一套内置工具，但您也可以通过[自定义工具](/docs/custom-tools)或 [MCP 服务器](/docs/mcp-servers)来扩展它。

默认情况下，所有工具都是**启用**的，并且不需要运行权限。您可以通过[权限](/docs/permissions)控制工具的行为。
...
[了解更多](/docs/permissions)关于配置权限的信息。
...
要配置项目中可用的 LSP 服务器，请参阅 [LSP 服务器](/docs/lsp)。
...
加载一个 [技能](/docs/skills)（一个 `SKILL.md` 文件）并在对话中返回其内容。
...
此工具默认对子智能体（subagents）禁用，但您可以手动启用它。[了解更多](/docs/agents/#permissions)
...
[了解更多](/docs/custom-tools)关于创建自定义工具的信息。
...
[了解更多](/docs/mcp-servers)关于配置 MCP 服务器的信息。
