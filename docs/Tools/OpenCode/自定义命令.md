...
自定义命令是对内置命令（如 `/init`、`/undo`、`/redo`、`/share`、`/help`）的补充。
...
使用 *!`command`* 将 bash 命令的输出注入到您的提示词中。
...
使用 `agent` 配置可选择指定哪个智能体应执行此命令。
如果这是一个子智能体，该命令默认将触发子智能体调用。
...
使用 `subtask` 布尔值强制命令触发子智能体调用。
如果您希望命令不污染您的主要上下文，这将非常有用，并且将**强制**智能体作为子智能体运行，
即使在智能体配置中 `mode` 设置为 `primary`。
...
opencode 包含几个内置命令，如 `/init`、`/undo`、`/redo`、`/share`、`/help`。
