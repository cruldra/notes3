...
![image](https://github.com/cruldra/picx-images-hosting/raw/master/image.7zqy83y5im.png)
...
系统通过 **"本期营"** 与 **"长期池"** 的双视图机制，解决 120人/期 的高频跟进与海量沉淀客户的管理难题。

#### 1. 双池自动流转逻辑
*   **入营 (Stage: Onboarding)**：Webhook 监听到"添加好友"或"进群"事件 -> 写入 `CurrentBatch` 表 -> 标记 `Stage=DAY0`。
*   **出营 (Stage: Graduation)**：
    *   **触发时机**：每周日晚 24:00（营期结束）。
    *   **执行动作**：系统扫描 `CurrentBatch` 中未成交且未标记"重点跟进"的客户 -> 批量移动至 `LongTermPool` 表 -> 状态更新为 `Archived`。
    *   **保留机制**：对于 S/A 量级的高意向客户，销售可手动"延期出营"，保留在本期列表中继续跟进。
*   **沉默判定 (Stage: Silent)**：
    *   **规则**：`LongTermPool` 中 `last_interaction_time < NOW - 30 days`。
    *   **动作**：标记 `Status=Silent`，移入"沉睡库"，降低数据同步频率以节省资源。

#### 2. 高性能列表查询优化
*   **PostgreSQL 分区表**：长期池数据按月份 (`partition_by_range`) 进行物理分区，确保百万级数据查询响应 < 200ms。
*   **Redis Bitmap**：记录每日活跃用户 ID，实现毫秒级"最近活跃"筛选。
*   **混合排序算法**：
    ```python
    # 动态权重排序
    final_score = (ai_score * 0.7) + (recency_weight * 0.3)
    # recency_weight: 今天=100, 3天内=80, 7天内=50, 其他=10
    ```

### 4.5 智能提醒引擎设计

不再依赖销售人工记忆，系统通过 **事件驱动 + 时间轮** 机制实现精准提醒。

#### 1. 优先级队列设计
| 优先级 | 队列 (Redis List) | 调度频率 | 场景示例 |
| :--- | :--- | :--- | :--- |
| **P0 (Urgent)** | `queue:remind:p0` | 实时 (Stream) | S量今日未跟进、收单倒计时、客户未回复 |
| **P1 (High)** | `queue:remind:p1` | 5分钟 | A量超2天未跟进、规划卡片未处理 |
| **P2 (Normal)** | `queue:remind:p2` | 30分钟 | B量超3天未跟进、批量群发任务 |
| **P3 (Low)** | `queue:remind:p3` | 每日 9:00 | 长期池沉默预警、生日提醒 |

#### 2. 动态规则引擎 (Celery Beat)
```python
# 伪代码：检测未回复预警
def check_unreplied_messages():
    # 查找30分钟前客户发来消息，且之后销售无回复的会话
    alerts = db.query(ChatSession).filter(
        last_sender='customer',
        last_msg_time < now() - timedelta(minutes=30),
        status='open'
    ).all()
    
    for alert in alerts:
        # 推送 P0 级提醒到企微侧边栏
        push_notification(alert.sales_id, level='Urgent', msg=f"{alert.customer_name} 等待回复中")
```

### 4.6 客户 360° 视图数据聚合 (BFF)

#### 1. 数据聚合层设计
前端请求 `/api/customers/{id}/profile` 时，后端采用 **并行聚合 (Fan-out/Fan-in)** 模式拉取多源数据：

*   **基础信息 (PostgreSQL)**：0.05s，获取姓名、标签、来源渠道、当前阶段。
*   **行为轨迹 (MongoDB)**：0.1s，聚合查询直播观看时长、历史聊天摘要、关键节点事件。
*   **AI 分析结果 (Redis Cache)**：0.02s，获取最新的雷达图评分、AI 建议摘要。
*   **企微状态 (WeCom API)**：0.2s，实时校验好友状态（是否删好友/拉黑）。

**总响应目标**：< 500ms

#### 2. 预加载策略
*   **列表悬停预加载**：当销售在列表页鼠标悬停某客户头像超过 300ms 时，Tauri 前端静默发起详情预加载请求。
*   **本地缓存**：客户的基础画像（不变数据）缓存在本地 SQLite，仅增量拉取最新的动态数据（聊天、行为）。
...
gantt
    ...
    环境搭建 & 数据库设计       :infra1, 2025-01-01, 10d
...
本项目研发投入预算为 **¥620,000** (62万元)，周期为 4 个月。我们将组建全栈闭环团队，根据项目不同阶段的实际需求，灵活配置以下专业角色投入：

| 关键角色 | 核心职责 |
| :--- | :--- |
| **系统架构师** | **技术总负责**。负责架构把控、技术选型、核心难点攻关（全程核心投入）。|
| **大模型应用工程师** | **AI 核心**。负责 LangGraph 编排、DSPy 提示词调优、RAG 知识库构建（全程核心投入）。 |
| **后端工程师** | **业务逻辑**。负责 FastAPI 微服务、企微对接、数据库设计与性能优化（全程核心投入）。 |
| **前端工程师** | **交互体验**。负责 React 后台、Tauri 侧边栏、数据可视化开发（中期重点投入）。 |
| **测试工程师** | **质量保障**。负责用例编写、自动化测试、压力测试、UAT 验收（中后期重点投入）。 |
| **研发总计** | **打包总价：¥620,000** |

> **注**：
> 1. 费用为项目总包报价，含研发人力、管理成本及 3 个月免费维保服务。
> 2. 不含云服务器、AI Token 费、企微接口费等第三方运营成本。
...
#### 1. 软件交付
- **容器化部署包**：提供编译后的 Docker 镜像（Image Tarballs）及 `docker-compose.yml` 或 K8s Helm Chart，确保在客户环境一键拉起所有服务。
- **数据库初始化包**：PostgreSQL/MongoDB/Qdrant 的 Schema 脚本及初始化数据。
- **客户端安装包**：Tauri 企微侧边栏应用的 Windows (.msi) 安装包。
...
- **AI 效果验证报告**：基于历史脱敏数据的回测报告，验证 AI 意向评分与资深销售判断的逻辑一致性，并提供持续优化方案。
