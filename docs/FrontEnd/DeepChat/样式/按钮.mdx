...
### `customButtons`[​](#customButtons "customButtons 的直接链接")

```
类型: { `styles?:` {[`button?: CustomButtonStyles`](#CustomButtonStyles), [`dropup?: CustomDropupItemStyles`](#CustomDropupItemStyles)}, [`position?: ButtonPosition`](#ButtonPosition), [`tooltip?: Tooltip`](#Tooltip), [`initialState?: CustomButtonState`](#CustomButtonState), [`setState?: SetCustomButtonState`](#SetCustomButtonState), [`onClick?: OnClickCustomButton`](#OnClickCustomButton) }[]
```
用于定义自定义按钮对象配置的数组。
`styles` 用于定义[按钮](#CustomButtonStyles)和[上拉菜单项](#CustomDropupItemStyles)的样式选项。
`position` 用于设置按钮位置。
`tooltip` 为按钮添加工具提示。
...
### `submitButtonStyles`[​](#submitButtonStyles "submitButtonStyles 的直接链接")

```
类型: { [`submit?: ButtonStyles`](#ButtonStyles), [`loading?: ButtonStyles`](#ButtonStyles), [`stop?: ButtonStyles`](#ButtonStyles), [`disabled?: ButtonStyles`](#ButtonStyles), `alwaysEnabled?: boolean`, [`position?: ButtonPosition`](#ButtonPosition), [`tooltip?: Tooltip`](#Tooltip) }
```
提交按钮的自定义样式。
`submit` 状态在用户输入有效且可以发送时显示。
`loading` 在等待来自目标服务器的消息响应时使用。
`stop` 在消息响应正在 [`流式传输`](/docs/connect#Stream)时使用。
`disabled` 在用户输入无法发送（参见 [`validateInput`](/docs/interceptors#validateInput)）或 [websocket](/docs/connect#Websocket) 连接未激活时使用。
您还可以通过调用 [`disableSubmitButton`](/docs/methods#disableSubmitButton) 方法手动设置此状态。
...
#### SVG 示例[​](#svg-example "SVG 示例的直接链接")
...
#### 组合示例[​](#combination-example "组合示例的直接链接")
...
## 自定义按钮类型[​](#custom-button-types "自定义按钮类型的直接链接")

自定义按钮属性共享的类型：

### `CustomButtonState`[​](#CustomButtonState "CustomButtonState 的直接链接")

*   类型: "default" | "active" | "disabled"

自定义按钮的可用状态。

### `CustomButtonStyles`[​](#CustomButtonStyles "CustomButtonStyles 的直接链接")

```
类型: {[`default?: ButtonStyles`](#ButtonStyles), [`active?: ButtonStyles`](#ButtonStyles), [`disabled?: ButtonStyles`](#ButtonStyles)}
```
自定义按钮状态的样式。
...
### `CustomDropupItemStyles`[​](#CustomDropupItemStyles "CustomDropupItemStyles 的直接链接")

```
类型: { [`default?: CustomDropupItemStateStyles`](#CustomDropupItemStateStyles), [`active?: CustomDropupItemStateStyles`](#CustomDropupItemStateStyles), [`disabled?: CustomDropupItemStateStyles`](#CustomDropupItemStateStyles) }
```
自定义上拉菜单项状态的样式。

### `CustomDropupItemStateStyles`[​](#CustomDropupItemStateStyles "CustomDropupItemStateStyles 的直接链接")

```
类型: {[`item?: StatefulStyles`](/docs/styles#StatefulStyles), [`iconContainer?: CustomStyle`](/docs/styles#CustomStyle), [`text?: CustomStyle`](/docs/styles#CustomStyle)}
```

上拉菜单中自定义项的样式。
上拉菜单项的 *svg 图标*和*文本*通过 [`CustomButtonStyles`](#CustomButtonStyles) 中的 `content` 属性定义。
有关默认和其他上拉菜单样式，请参见 [`dropupStyles`](/docs/styles/#dropupStyles)。
...
### `SetCustomButtonState`[​](#SetCustomButtonState "SetCustomButtonState 的直接链接")

*   类型: ([`newState: CustomButtonState`](#CustomButtonState)) => `void`
...
### `OnClickCustomButton`[​](#OnClickCustomButton "OnClickCustomButton 的直接链接")

*   类型: ([`currentState: CustomButtonState`](#CustomButtonState)) => [`CustomButtonState`](#CustomButtonState) | `void`

分配一个在单击自定义按钮时触发的函数。
您可以选择返回一个 [`CustomButtonState`](#CustomButtonState) 字符串来为按钮赋予新状态。
...
### `Button`[​](#Button "按钮的直接链接")

```
类型: {[`styles?: ButtonStyles`](#ButtonStyles), [`position?: ButtonPosition`](#ButtonPosition), [`tooltip?: Tooltip`](#Tooltip)}
```
聊天视图中按钮的样式首选项。
`styles` 定义按钮样式。
`position` 用于设置按钮位置。
...
### `ButtonStyles`[​](#ButtonStyles "ButtonStyles 的直接链接")

```
类型: { [`container?: StatefulStyles`](/docs/styles#StatefulStyles), [`svg?: ButtonInnerStyles`](#ButtonInnerStyles), [`text?: ButtonInnerStyles`](#ButtonInnerStyles) }
```
...
您可以使用 [`CSSFilterConverter`](https://cssfilterconverter.com/) 工具为图标颜色生成滤镜值。

### `ButtonPosition`[​](#ButtonPosition "ButtonPosition 的直接链接")

*   类型: "inside-start" | "inside-end" | "outside-start" | "outside-end" | "dropup-menu"

按钮的位置，可以在[文本输入容器](/docs/styles#textInput) *内部*，*外部*（*左*或*右*）或[`上拉菜单`](/docs/styles#dropupStyles)内部。

#### 示例[​](#example-6 "示例的直接链接")
...
### `MicrophoneStyles`[​](#MicrophoneStyles "MicrophoneStyles 的直接链接")

```
类型: { [`default?: ButtonStyles`](#ButtonStyles), [`active?: ButtonStyles`](#ButtonStyles), [`unsupported?: ButtonStyles`](#ButtonStyles), [`position?: ButtonPosition`](#ButtonPosition), [`tooltip?: Tooltip`](#Tooltip) }
```
麦克风按钮的自定义样式。
`default` 用于默认按钮状态。
`active` 在麦克风激活时使用。
