...
您可以通过在 connect 属性中定义其详细信息来连接到任何 API。目标端点需要能够使用下面描述的格式进行接收和响应。
...
来自目标服务器的响应需要使用 Response JSON 类型。

> **提示**
>
> 如果您不想/无法更改目标服务器来处理所需的对象类型，请使用 interceptor（拦截器）属性来增强传输的对象，或使用 handler 函数来控制请求代码。
...
```
类型: {MessageContent, `error?: string`, `overwrite?: boolean`} | `Array`
```

包含来自目标服务响应信息的对象。它具有与 MessageContent 相同的属性，并附加了可选的 `error` 和 `overwrite` 属性：
`text` 是文本消息的内容。
`files` 是一个封装响应件详情的数组。
`html` 是定义自定义元素标记的字符串。它必须描述完整的元素。
`error` 描述有关服务器错误的信息。如果 errorMessages 中的 *displayServiceErrorMessages* 属性设置为 *true*，则相同的消息将显示在聊天的错误气泡中。
`overwrite` 替换来自同一角色的最后一条消息，如果未找到则创建一个新消息。状态气泡示例见下文。
Deep Chat 也接受 `Array` 中的多个 `Response` 对象，但是 Stream 不支持此功能。
...
这用于与您的服务器建立 websocket 连接。通过在 connect 对象中将 `websocket` 属性定义为 `boolean` *true* 或字符串类型的连接协议（或多个协议的字符串数组）来启用它。
...
> **注意**
>
> 当定义了 demo 时，可以在 connect 中单独使用 `stream`。

### `Handler`

*   类型: (`body: any`, `signals: Signals`) => `void`

此函数让开发人员可以完全控制使用自己的代码发出服务器请求。
当用户尝试发送消息时会调用此函数，它包含两个核心参数：
`body` 是一个包含发出消息详情的对象，使用 Request message 类型。
