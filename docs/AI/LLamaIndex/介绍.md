LlamaIndex 是一个领先的框架，用于利用 [LLM](https://en.wikipedia.org/wiki/Large_language_model) 和 [工作流](/python/llamaagents/workflows) 基于您的数据构建 LLM 驱动的智能体。

### 什么是智能体？

[智能体](/python/framework/understanding/agent) 是由 LLM 驱动的知识助手，利用工具执行研究、数据提取等任务。智能体的范围从简单的问答系统到能够感知、决策并采取行动以完成任务的复杂系统。

LlamaIndex 提供了一个构建智能体的框架，包括将 RAG 管道作为众多工具之一来完成任务的能力。

### 什么是工作流？

[工作流](/python/llamaagents/workflows) 是多步骤过程，结合了一个或多个智能体、数据连接器和其他工具来完成任务。它们是事件驱动的软件，允许您结合 RAG 数据源和多个智能体来创建一个复杂的应用程序，能够执行各种任务，具有反思、纠错和其他高级 LLM 应用程序的特征。您可以随后将这些[智能体工作流部署](/python/workflows/deployment/)为生产微服务。
...
上下文增强最流行的例子是[检索增强生成或 RAG](/python/framework/getting_started/concepts)，它在推理时将上下文与 LLM 相结合。
...
* [问答](/python/framework/use_cases/q_and_a)（检索增强生成，即 RAG）
* [聊天机器人](/python/framework/use_cases/chatbots)
* [文档理解和数据提取](/python/framework/use_cases/extraction)
* [自主智能体](/python/framework/use_cases/agents)，可以执行研究并采取行动
* [多模态应用程序](/python/framework/use_cases/multimodal)，结合文本、图像和其他数据类型
* [微调](/python/framework/use_cases/fine_tuning)模型以提高性能

查看我们的[用例](/python/framework/use_cases)文档以获取更多示例和教程链接。
...
LlamaIndex 提供 Python（本文档）和 [Typescript](https://ts.llamaindex.ai/) 版本。如果您不确定从哪里开始，我们建议阅读[如何阅读这些文档](/python/framework/getting_started/reading)，它将根据您的经验水平为您指明正确的方向。
...
如果其中任何部分让您感到困惑，请不要担心！查看我们更全面的入门教程，使用[远程 API 如 OpenAI](/python/framework/getting_started/starter_example) 或 [任何在您笔记本电脑上运行的模型](/python/framework/getting_started/starter_example_local)。
