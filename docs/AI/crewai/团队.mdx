...
按照安装部分的说明创建 CrewAI 项目后，您可以在继承自 `CrewBase` 的类中定义 Crew，并使用装饰器来定义 Agent、任务和 Crew 本身。
...
这些方法为您管理和执行 Crew 内的任务提供了灵活性，允许根据您的需求定制同步和异步工作流。有关详细的异步示例，请参阅异步启动 Crew 指南。
