...
`transformers.activations` 是 Hugging Face Transformers 库中的核心模块，负责管理和提供各种神经网络[激活函数](/AI/相关概念/激活函数.md)。该模块不仅封装了 PyTorch 原生的激活函数（如 ReLU, Tanh），还提供了在大语言模型（LLM）和 Transformer 架构中广泛使用的特定激活函数的优化实现（如 GELU 的多种变体、Mish、Swish/SiLU 等）。
