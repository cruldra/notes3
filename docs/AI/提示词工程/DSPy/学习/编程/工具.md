...
你可以启用 [MLflow 跟踪](https://dspy.ai/tutorials/observability/) 来检查原生工具调用的使用情况。如果你使用启用了原生函数调用的 `JSONAdapter` 或 `ChatAdapter` 运行[上一节](tools.md#basic-setup)中提供的代码片段，你应该会看到原生函数调用参数 `tools` 设置如下图所示：
